"""
Trade Journal — Logs every trade with full context.
...
class TradeJournal:
    ...
    def close_trade(
        ...
        self._append_csv(r)
        logger.info(f"JOURNAL CLOSE | {trade_id} | pnl={r.pnl:.2f} ({r.pnl_pct:.2f}%) | reason={exit_reason}")
        return r
    ...
    def summary(self, agent_id: str = None) -> dict:
        ...
        return {
            ...
            "profit_factor": round(abs(sum(winners) / sum(losers)), 2) if losers and sum(losers) != 0 else 0,
